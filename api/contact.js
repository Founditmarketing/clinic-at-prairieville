...
module.exports = async (req, res) => {
  ...
  if (!name || !email) {
    res.statusCode = 303;
    res.setHeader("Location", "/?error=missing");
    return res.end();
  }

  const to   = process.env.CONTACT_TO_EMAIL   || "prairieville@theclinics.us";
  const from = process.env.CONTACT_FROM_EMAIL || "The Clinic at Prairieville <leads@theclinicatprairieville.com>";

  const subject = `New lead from theclinicatprairieville.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on theclinicatprairieville.com`;
  const html = `
<h2>New lead from The Clinic at Prairieville</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on theclinicatprairieville.com</p>`;
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      res.statusCode = 303;
      res.setHeader("Location", "/?error=send");
      return res.end();
    }
  } catch (e) {
